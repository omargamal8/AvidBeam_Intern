...
def hwEstimate(depScen):
  ...
  for m in media:
    print('dd', m)
    med= m['media_path']
    with open("conf.json", "r") as fromm, open(str(i) + ".json", "w") as to:
      data=json.load(fromm)
      to.write(json.dumps(data))
...
def addMediaSource(depMod,filewin):
  ...
  medcont=[]
  save = Button(filewin, text ="Add",bg=mycolor ,fg='white',command= lambda: addMediaSrc(depMod,e1.get(),e2.get(),e3.get(),e4.get(),e5.get(),e6.get(),var.get(),e7.get() ,medcont,filewin))
  save.grid(row=20, columnspan=20)
...
def addMediaSource2(depScen,filewin):
   ...
   e7.grid(row=6, column=1)
   mediacont=[]
   save = Button(filewin, text ="Add",bg=mycolor ,fg='white',command= lambda: addMediaSrc2(depScen,e1.get(),e2.get(),e3.get(),e4.get(),e5.get(),e6.get(),var.get(),e7.get() ,mediacont,filewin))
   save.grid(row=20, columnspan=20)
